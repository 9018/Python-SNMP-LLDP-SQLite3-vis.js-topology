...
def main():
    ...
    for switch in switches_filedata:
        ...
        for id, LocPortId_hex in lldpLocPortId.items():
            if LocPortId_hex.startswith('Hex-STRING:'):
                LocPortId_hex = LocPortId_hex.split(':')[1].replace(' ', '')  # 去掉 'Hex-STRING:' 和空格
                LocPortId = bytearray.fromhex(LocPortId_hex).decode('utf-8', 'ignore')
            else:
                try:
                    LocPortId = LocPortId_hex.split(':')[1].strip(' "\"')
                except IndexError:
                    LocPortId = LocPortId_hex.strip(' \"')
            print('id=', id)
        ...
        for id, RemSysName in lldpRemSysName.items():
            ...
            for id, RemPortId_hex in lldpRemPortId.items():
                if RemPortId_hex.startswith('Hex-STRING:'):
                    RemPortId_hex = RemPortId_hex.split(':')[1].replace(' ', '')  # 去掉 'Hex-STRING:' 和空格
                    RemPortId = bytearray.fromhex(RemPortId_hex).decode('utf-8', 'ignore')
                else:
                    try:
                        RemPortId = RemPortId_hex.split(':')[1].strip(' "\"')
                    except IndexError:
                        RemPortId = RemPortId_hex.strip(' \"')
                print('id=', read_id_from_oid_tail(id, with_len=False))
                print('操你妈RemPortId=', RemPortId)
                cursorObj.execute(
